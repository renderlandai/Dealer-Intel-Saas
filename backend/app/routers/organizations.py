...
async def update_org_settings(org_id: UUID, updates: OrgSettingsUpdate, user: AuthUser = Depends(get_current_user)):
...
async def test_email(org_id: UUID):
...
async def get_org_logo(org_id: UUID):
...
async def upload_org_logo(org_id: UUID, file: UploadFile = File(...)):
    """
...
async def delete_org_logo(org_id: UUID):
